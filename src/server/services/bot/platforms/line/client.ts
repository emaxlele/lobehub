...
class LineWebhookClient implements PlatformClient {
  ...
  async extractFiles(message: Message): Promise<AttachmentSource[] | undefined> {
    const raw = (message as any).raw as LineMessage | undefined;
    const messageId = resolveMediaMessageId(raw);
    if (!raw || !messageId) return undefined;

    log('extractFiles: msgId=%s lineMsgId=%s', (message as any).id, messageId);
